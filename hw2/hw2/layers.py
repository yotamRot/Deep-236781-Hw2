...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        dx = dout * minus_exp / torch.pow((1 + minus_exp), 2)
        # ========================
...
class TanH(Layer):
    ...
    def forward(self, x, **kw):
        ...
        minus_exp = torch.exp(-x)
        exp = torch.exp(x)
        out = (exp-minus_exp)/(exp+minus_exp)
        self.grad_cache["x"] = x
        # ========================
    ...
    def backward(self, dout):
        ...
        minus_exp = torch.exp(-x)
        exp = torch.exp(x)
        dx = dout * 4 / (torch.pow((exp + minus_exp), 2))
        # ========================
